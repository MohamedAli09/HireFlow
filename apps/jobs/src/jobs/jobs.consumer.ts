...
export class JobsConsumer {
  ...
  async handleApplicationCreated(
    ...
    try {
      ...
      if (!job) {
        await this.publishCompensation(
          event.applicationId,
          `Job #${event.jobId} not found`,
          event.correlationId,
        );
        return;
      }
      ...
      if (!job.isActive) {
        await this.publishCompensation(
          event.applicationId,
          `Job #${event.jobId} is no longer active`,
          event.correlationId,
        );
        return;
      }
      ...
      logger.error(`Failed to update applicant count: ${err.message}`);
      const isTransient =
  ...
  private async publishCompensation(
    ...
    logger.warn(
      `Compensation event published for application #${applicationId}: ${reason}`,
    );
  }
}
